...
namespace Core
{
    public static class OktwCommon
    {
        ...
        private static bool CanHitSkillShot(Obj_AI_Base target, GameObjectProcessSpellCastEventArgs args)
        {
        ...
        {
            foreach (var buff in target.Buffs.Where(buff => buff.Name.ToLower() == buffName.ToLower(CultureInfo.CurrentCulture)))
            {
                if (buff.Count == 0)
                    return 1;
                return buff.Count;
        ...
        public static float GetPassiveTime(Obj_AI_Base target, string buffName)
        {
            return
                target.Buffs.OrderByDescending(buff => buff.EndTime - Game.Time)
                    .Where(buff => buff.Name.ToLower() == buffName.ToLower(CultureInfo.CurrentCulture))
        ...
        public static Vector3 GetTrapPos(float range)
        {
            ...
            {
                var name = obj.Name.ToLower(CultureInfo.CurrentCulture);

                if (name.Contains("GateMarker_red.troy".ToLower(CultureInfo.CurrentCulture)) || name.Contains("global_ss_teleport_target_red.troy".ToLower(CultureInfo.CurrentCulture))
                    || name.Contains("R_indicator_red.troy".ToLower(CultureInfo.CurrentCulture)))
                    return obj.Position;
            }
